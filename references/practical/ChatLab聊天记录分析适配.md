...
ChatLab负责导入、检索、统计和返回带编号的聊天证据；关系罗盘负责情绪承接、事实／推测拆分、关系判断与行动建议。适配不赋予关系罗盘直接读取或导出微信、QQ等应用数据库的能力。
...
[Relationship Candlestick Lab](https://github.com/ZhenyuanPAN822/relationship-candlestick-lab) 可作为可选探索层。只吸收按时间聚合、关键转折、事件归因和代表性消息编号；K线价格、MACD、RSI、KDJ、均值回归和通用固定基线不是经过验证的关系测量，不能据此判断爱意、忠诚或分手概率，也不能写入稳定档案。用户想看趋势时优先展示可解释的主动频率、回复间隔、邀约兑现、冲突与修复事件，并把最近窗口与同一对象的既有互动 baseline 比较；模型评分必须显式标注主观性。

## 回答与记忆边界

回答开头说明实际查询的会话和时间范围。引用 `[#1021]` 等证据编号，分开写：

1. 可观察事实：2–5 条关键主动、兑现、时间间隔或明确原话；普通分析不逐句点评。
2. 合理解释：可能的关系含义、其他解释和冲突证据。
3. 关系状态：Stage 与相对 baseline 的 Recent Trend 分开，并用定性语言说明 Evidence Strength。
4. 关键未知：线下互动、没有说出口的意图、应用外事件或缺失 baseline。
5. 建议：一个低成本当前动作、观察窗口和停止条件。
