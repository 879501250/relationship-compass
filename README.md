...
- 关系分析会区分当前阶段与近期走势，并优先依据持续、现实和明确证据，而不是单次晚回或短回复。
- 仅在当前决策确实缺少关键上下文时提出一个高信息量问题，吸收回答并在足够后立即停止。
- 回复先从有限动作集合中只选一个本轮主动作，再决定措辞、气泡、幽默或 hook；好素材不会覆盖边界、serious need、证据充分性与 continuation ownership。
- 回复建议优先保持用户本人的表达习惯并只做一个自然小跨度；用户已有合适草稿时默认确认或微调，而不是完全重写。
