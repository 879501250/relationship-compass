...
本模块只在收到 Decision Handoff 后校准 wording、directness、specificity、emotional exposure、style、Comfortable Range、One Small Stretch 与 assistance strategy。它不得因为“需要练习主动／幽默／分享”而把 `LEAVE_SPACE`、`CLOSE` 或 `WAIT` 改成 `ASK`、`SHARE`、`PLAY` 或其他动作；成长目标不能反向授权 action。stretch 与 action fidelity、hard constraints、stop conditions、ownership、serious 或 boundary 冲突时直接取消跨度；无法在当前动作内合规实现时返回 Decision Layer。
